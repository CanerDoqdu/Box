 ...
 * }>}
 */
export async function executeGuardrailAction(config, action, scenarioId, reasonCode, opts = {}) {
 ...
 * }>}
 */
export async function executeGuardrailsForDetections(config, detections, opts = {}) {
  const t0     = Date.now();
  const dryRun = Boolean(opts?.dryRun);
