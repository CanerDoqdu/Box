...
describe("policy_engine", () => {
  ...
  it("matches protected prefix glob patterns", () => {
    ...
    assert.equal(isProtectedPath(policy, "src/dashboard/live_dashboard.js"), false);
  });
  ...
  describe("path matching — **/ prefix (filename anywhere) patterns", () => {
    ...
    it("matches files by extension pattern (**/*.test.js)", () => {
      const policy = { protectedPaths: ["**/*.test.js"] };
      assert.equal(isProtectedPath(policy, "tests/core/policy_engine.test.js"), true);
      assert.equal(isProtectedPath(policy, "policy_engine.test.js"), true);
    });

    it("does not match non-test-js files against **/*.test.js", () => {
      const policy = { protectedPaths: ["**/*.test.js"] };
    ...
    it("matches deeply nested file by extension pattern", () => {
      const policy = { protectedPaths: ["**/*.test.js"] };
      assert.equal(isProtectedPath(policy, "a/b/c/d/file.test.js"), true);
    });
  });
  ...
  describe("no false positives for unrelated files", () => {
    it("unrelated files do not match protected path patterns", () => {
      ...
      assert.equal(isProtectedPath(policy, "src/dashboard/live.js"), false);
      assert.equal(isProtectedPath(policy, "tests/core/policy_engine.test.js"), false);
    });
