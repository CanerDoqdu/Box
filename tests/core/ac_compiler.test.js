...
describe("ac_compiler", () => {
  ...
  describe("enrichPlansWithAC", () => {
    ...
    it("populates acceptance_criteria when enriching a weak plan", () => {
      const plans = [{ task: "Add trust-boundary provider integration tests for untrusted linter payloads", target_files: ["tests/core/trust_boundary_provider_integration.test.js"] }];
      const result = enrichPlansWithAC(plans);
      assert.ok(Array.isArray(result.plans[0].acceptance_criteria));
      assert.ok(result.plans[0].acceptance_criteria.length >= 2);
      assert.ok(result.plans[0].verification.includes("node --test tests/core/trust_boundary_provider_integration.test.js"));
    });
  });
});
