 ...
 *
 * Usage:
 *   node scripts/run_evolution.mjs                  # start / resume
 *   node scripts/run_evolution.mjs --from T-005     # start from specific task
 *   node scripts/run_evolution.mjs --dry-run        # print plan, skip workers
 *   node scripts/run_evolution.mjs --status         # show progress summary
 */
...
if (showStatus) {
  ...
  const { cycle_id, tasks, current_task_index } = progress;
...
try {
  const result = await runEvolutionLoop(config, { fromTaskId, dryRun });
