...
function listDashboardPids() {
  if (process.platform === "win32") {
    const output = runSafe(
      "powershell -NoProfile -Command \"Get-CimInstance Win32_Process | Where-Object { $_.Name -eq 'node.exe' -and $_.CommandLine -match 'src[\\\\/]dashboard[\\\\/]live_dashboard\\.js' } | Select-Object -ExpandProperty ProcessId\""
    );
  ...
  return String(output || "")
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter((line) => /node/.test(line) && /src[\\/]dashboard[\\/]live_dashboard\.js/.test(line))
    .map((line) => Number(line.split(/\s+/)[0]))
    .filter((pid) => Number.isFinite(pid) && pid > 0);
}

function requestDaemonStop() {
  const result = spawnSync("node", ["src/cli.js", "stop"], {
    cwd: ROOT,
    stdio: "inherit",
    env: process.env
  });
